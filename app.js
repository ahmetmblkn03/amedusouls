...
function renderChecklist(areas, gameKey) {
    ...
    areas.forEach((area, areaIndex) => {
        ...
        taskArrayForContent.forEach((step, stepIndex) => {
            ...
            let labelType = step.type;
            
            let locationHtml = step.location ? `<div class="task-location"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg> ${step.location}</div>` : '';

            let html = `
                <span class="task-type-badge">${labelType}</span>
